...
/*   Updated: 2024/12/19 10:31:47 by vpozniak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	test_ft_memchr(const void *s, int c, size_t n)
{
	...
	if (result != NULL)
	{
		printf("Found at: %s\n", result); // If found,print the rest of the string starting from the match
	}
	else
	{
...
int	main(void)
{
	...
	printf("\nTest Case 7: Searching for 'w' within first 5 characters of \"%s\"\n",
		str7);
	test_ft_memchr(str7, 'w', 5); // Searching within first 5 characters,'w' is at index 7
